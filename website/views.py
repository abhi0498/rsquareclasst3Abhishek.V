...
def view_courses(request):
    courses = models.Course.objects.filter(
        branch__name=request.session['curr_branch'])
    ...
    if request.method == 'GET':
        ...
        if request.user.is_owner():
            ...
            if add_to_all == 'true':
                for branch in models.Branch.objects.all():
                    ...
                    for subject_id in request.POST.getlist('subjects[]'):
                        courseSubject = models.CourseSubject.objects.create(
                            course=course, subject_id=subject_id)
            else:
                course = models.Course.objects.create(name=name, fee=fee,
                                                      duration=duration, no_of_installments=no_of_installments,
                                                      days_between_two_installments=days_between_two_installments,
            ...
            course = models.Course.objects.create(name=name, fee=fee, duration=duration, no_of_installments=no_of_installments, days_between_two_installments=days_between_two_installments,
                                                  branch=request.user.branch, duration_type=duration_type, installment_duration_type=installment_duration_type)
            for subject_id in request.POST.getlist('subjects[]'):
                courseSubject = models.CourseSubject.objects.create(
                    course=course, subject_id=subject_id)
...
def student_add_new(request):
    if not (request.user.is_owner() or request.user.is_branch_manager()):
        return redirect('dashboard')
    if request.method == 'POST':
        curr_branch = models.Branch.objects.get(
            name=request.session['curr_branch'])
        ...
        return redirect('student-guardian-view', pk=student.id)
    else:
    ...
    if request.method == 'POST':
        guardian = models.Guardian.objects.create(student_id=student.profile.pk, relation=request.POST['relation'], salutation=request.POST['salutation'],
                                                  first_name=request.POST['first_name'], last_name=request.POST[
                                                      'last_name'], aadhar_number=request.POST['aadhar_number'],
                                                  phone=request.POST['phone'], email=request.POST['email'], occupation=request.POST['occupation'])
        return HttpResponseRedirect(request.path_info)
    else:
        guardians=models.Guardian.objects.all().filter(student_id=student.profile.pk)
        cont={'guardians':guardians.values(),
        'pk':pk}
        print(guardians)
        return render(request, 'student/guardian.html',context=cont)
...
def student_courses(request, pk):
    ...
    if request.method == 'POST':
        ...
        student_course = models.StudentCourse.objects.create(admission_date=request.POST['admission_date'],
                                                             discount=request.POST['discount'], discount_type=request.POST[
                                                                 'discount_type'], notes=request.POST['notes'],
